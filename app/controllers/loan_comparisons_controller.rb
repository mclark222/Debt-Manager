class LoanComparisonsController < ApplicationController
  ...
  def create
    ...
    the_loan_comparison.user_id = session[:user_id]
    ...
    the_loan_comparison.loan_total_cost = (the_loan_comparison.total_loan_amount.to_i * ((1 + (the_loan_comparison.apr.to_i/100)) ** the_loan_comparison.repayment_period.to_i))
    the_loan_comparison.monthly_payments_after_school = the_loan_comparison.total_loan_amount.to_i / the_loan_comparison.repayment_period.to_i / 12
    the_loan_comparison.monthly_payments_in_school = 0

    if the_loan_comparison.valid?
      the_loan_comparison.save
      redirect_to("/loan_dashboard", { :notice => "Loan comparison created successfully." })
    else
  ...
  def destroy
    ...
    redirect_to("/loan_dashboard", { :notice => "Loan comparison deleted successfully."} )
  end
end
